...
<div class="block-right-filter">
	<div class="block-right-filter-content">
		<span>Показать заказы:</span>
		<div class="filter-param">
			<a href="/user/orders/" class="filter-param-item <?if($_REQUEST == false){echo 'active';}?>">активные</a>
			<a href="?show_all=Y" class="filter-param-item <?if($_REQUEST["show_all"] == 'Y'){echo 'active';}?>">все</a>
			<a href="?filter_history=Y" class="filter-param-item <?if($_REQUEST["filter_history"] == 'Y'){if($_REQUEST["show_canceled"] == false){echo 'active';}}?>">выполненные</a>
			<a href="?filter_history=Y&show_canceled=Y" class="filter-param-item <?if($_REQUEST["show_canceled"] == 'Y'){echo 'active';}?>">отмененные</a>
		</div>
	</div>
</div>
<div class="history-list">
	...
	else
	{
		...
		?>
		<div class="row col-md-12 col-sm-12" hidden>
			<?
			...
			foreach ($arResult['ORDERS'] as $key => $order)
			{
				...
				<div class="history-item">
					...
					<div class="history-item-content">
						<span class="sale-order-list-inner-title-line" style="display:none;">
						...
						foreach ($order['PAYMENT'] as $payment)
						{
							...
							?>
							<div class="history-col-2">
								<p><?=Loc::getMessage('SPOL_TPL_SUM_TO_PAID')?>: <b><?=$payment['FORMATED_SUM']?></b></p>
								<p>Оплачен:
									<?
									if ($payment['PAID'] === 'Y')
									{
								...
								<? foreach($order['BASKET_ITEMS'] as $basketItem){
									echo $i.". ".$basketItem['NAME'] ." - ".$basketItem['QUANTITY'] ."шт. <br />";
									$i++;
								}?>
							</div>
							<div class="history-right-box">
								<div class="history-col-1">
									<?if(isset($payment['DATE_BILL'])){
										echo "<p>".$payment['DATE_BILL']."</p>";
									};?>
									<?foreach ($order['SHIPMENT'] as $shipment)
									{
										...
										<div class="col-md-9 col-sm-8 col-xs-12 sale-order-list-shipment">
											<div class="sale-order-list-shipment-title" hidden>
											...
											if (!empty($shipment['DELIVERY_ID']))
											{
												...
												<div class="sale-order-list-shipment-item">
													<?=Loc::getMessage('SPOL_TPL_DELIVERY_SERVICE')?>:<br />
								...
								<div class="history-col-1">
									<a class="btn-order btn-order-1" href="<?=htmlspecialcharsbx($payment['PSA_ACTION_FILE'])?>">Оплатить</a>
									<a class="btn-order btn-order-2" href="<?=htmlspecialcharsbx($order["ORDER"]["URL_TO_COPY"])?>">Повторить</a>
									<a class="btn-order btn-order-3" href="<?=htmlspecialcharsbx($order["ORDER"]["URL_TO_CANCEL"])?>">Отменить</a>
								</div>
							</div>
							...
							<div class="row sale-order-list-inner-row">
								...
								<div class="sale-order-list-inner-row-body">
									<div class="col-md-9 col-sm-8 col-xs-12 sale-order-list-payment">
										...
										<div class="sale-order-list-payment-title" hidden>
											<?
						...
						{
							?>
							<div class="sale-order-list-inner-title-line" style="display:none;">
